...
public class QuizService : IQuizService
{
    ...
    private const int XP_BASE = 10;
    private const int XP_PER_CORRECT = 5;
    private const int XP_BONUS_3_STARS = 20;
    ...
    public async Task<QuizResultDto> SubmitQuizAsync(int userId, SubmitQuizRequestDto request)
    {
        ...
        if (activity == null)
        {
            activity = new UserActivity
            {
                ...
                IsCompleted = score >= THRESHOLD_2_STARS
            };
    ...
    public async Task<QuizDto?> UpdateQuizAsync(int quizId, CreateUpdateQuizDto dto)
    {
        ...
        foreach (var oldAnswer in quiz.Answers)
        {
            ...
            {
                await _cloudinaryService.DeleteImageAsync(oldAnswer.ImageUrl);
            }
        }
    ...
    private static void ValidateQuiz(CreateUpdateQuizDto dto)
    {
        ...
        {
            var a = dto.Answers[i];
            bool hasAnswerContent = !string.IsNullOrWhiteSpace(a.AnswerText)
                                  || a.ImageFile != null;
            if (!hasAnswerContent)
                throw new InvalidOperationException(
                    $"Đáp án {i + 1} phải có nội dung (chữ hoặc ảnh)!");
        }
    }
